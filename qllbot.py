...
try:
	...
	if CRASH_LOG_PATH != '':
		counter = 0
		while os.path.exists('%s/crash.%d.log' % (CRASH_LOG_PATH, counter)):
			counter += 1
		fp = open('%s/crash.%d.log' % (CRASH_LOG_PATH, counter), 'w')
		traceback.print_exception(type(exc), exc, sys.exc_info()[2], None, fp)
		fp.close()
	if PRINT_EXCEPTIONS:
		traceback.print_exception(type(exc), exc, sys.exc_info()[2])
